...
import { Row, Col, Modal, Collapse, Table, Form, Select, TreeSelect, Input, Button, Progress, Card, Rate, Timeline, Avatar, Icon, Popover, Tag, notification } from 'antd'
...
const height_screen = window.screen.availHeight - (window.outerHeight - window.innerHeight) - 370
...
class CRM extends Component {
    ...
    componentWillMount() {
        const { authen, GET_LEADTOPUP_SUMMARY, GET_MASTER_REGION, GET_MASTER_AREA, GET_MASTER_TEAM, GET_MASTER_EMPS  } = this.props
    ...
    componentWillReceiveProps(props) {
        if(props) {
            ...
            if(!_.isEmpty(data.leadTopUp)) { 
                _.map(data.leadTopUp, (v) => {
                    v.CustProfile = (<i className={`fa fa-television pointer ${cls['icon_click']}`} aria-hidden="true" onClick={this.handleCustProfile.bind(this, v)}></i>)
                })
            }

            if(this.state.createActivity) {
                const { LOAD_ACTIONNOTE } = props
    ...
    render() {
        ...
        const { config, columns, authen, master, data, activity_timeline, lead_topup_summary, LOAD_ACTIONNOTE, CREATE_ACTIONNOTE } = this.props
            ...
            <div className={`${cls['grid_container']}`}>
                ...
                <div className={`${cls['grid_header']}`}>
                    <div className={`${cls['grid_title']}`}>
                        <p>{config.grid.crm.lead_topup.header.title}</p>
                        <p>{`${config.grid.crm.lead_topup.header.subtitle} : 01/09/2018 - 31/12/2018`}</p>
                ...
                <Table 
                    ...
                    columns={columns.crm_customer_list} 
                    dataSource={data.leadTopUp} 
                    loading={(data.leadTopUp && data.leadTopUp.length > 0) ? false : true}
                    footer={this.handleFooter}
                    bordered={config.grid.border}
                    pagination={{ ...pagination }}
                    scroll={{ y: height_screen }}
    ...
    handleFooter = (currentPageData) => {
        ...
        const lead_topup = (data.leadTopUp && data.leadTopUp.length > 0) ? data.leadTopUp : []
        ...
        if(parentTable) {
            ...
            if(rtd_count > 0) {
                ...
                let footer_summary = { 
                    cur: {
                        0: 'Total',
                        3: numberWithCommas(largeNumberToShort(roundFixed(_.sumBy(currentPageData, 'TopUpFullAmt'), 0), 5)),
                    ...
                    all: {
                        0: 'Grand Total',
                        3: numberWithCommas(largeNumberToShort(roundFixed(_.sumBy(lead_topup, 'TopUpFullAmt'), 0), 5)),
    ...
    /***************************     SET HEADER     ***************************/
    handleHeadFilter = () => {
        const { config, form, columns, filters, lead_topup_summary } = this.props
        const { getFieldDecorator } = form

        const lead_summary_content = (
            <div className={cls['pcis_container']} style={{ width: '250px' }}>
                <h4 className="ttu">KPI Summary</h4>
                <div className={`${cls['grid_container']}`}>
                    <Table 
                ...
                <div className={`${cls['item_header']}`} data-attr="search-filter">
                    ...
                    <div className={cls['notice_dayleft']}>
                        <Popover placement="bottomRight" content={lead_summary_content}>
                            <Tag color="red" style={{ fontSize: '1em' }}><Icon type="clock-circle-o" /> 90 Days left</Tag>
                        </Popover>
                    </div>
                    ...
                    <div className={`${cls['panel_container']} ${cls['open']} ${cls['collapse_container']}`}>
                        <Collapse defaultActiveKey={this.state.ActiveFilter} activeKey={this.state.ActiveFilter} className={`${cls['collapse_filter']}`} onChange={this.handleCollapseFilter}>
                            <Panel key="1" header={<header><Icon type="search" /> {config.grid.default.panel_title}</header>}>
...
class CustProfile extends Component {
        ...
        if(props) {
            if(props.dataSource && !_.isEmpty(props.dataSource)) {
                if(_.isEmpty(this.state.data)) {
                    ...
                    if(dataSource.CIFNO && !_.isEmpty(dataSource.CIFNO)) {
                        loadActionNote({ CIFNO: dataSource.CIFNO })
    ...
    handleSearchSubmit = (e) => {
        ...
        validateFields((err, fieldData) => {
            if(!err) {
                ...
                let requestData = {
                    CIFNO: (dataSource.CIFNO && !_.isEmpty(dataSource.CIFNO)) ? dataSource.CIFNO : null,
                    CitizenID: (dataSource.Ac_CitizenID && !_.isEmpty(dataSource.Ac_CitizenID)) ? dataSource.Ac_CitizenID : null,
    ...
    render() {
        const { config, isOpen, form, master, dataSource, activityTimeline } = this.props
        ...
        let custphone = (data && !_.isEmpty(data.AC_Tel)) ? data.AC_Tel : ''
        ...
        let bureau_score = (data && !_.isEmpty(data.Bureau_Score) && data.Bureau_Score !== '#N/A') ? data.Bureau_Score : '?'
        let bureau_rate = (data && data.Bereau_Rate > 0) ? data.Bereau_Rate : 0
               
        // CUSTOMER INFORMATIOn
        let cust_address = (data && !_.isEmpty(data.AC_Address)) ? data.AC_Address : ''
        let channel = (data && !_.isEmpty(data.Channel)) ? data.Channel : '-'
        let team = (data && !_.isEmpty(data.BranchDigit)) ? data.BranchDigit : '-'
        let rm_orgin = (data && !_.isEmpty(data.Sale_OwnerName)) ? data.Sale_OwnerName : ''
        let rm_name = (data && !_.isEmpty(data.Sale_CurrentName)) ? data.Sale_CurrentName : ''
        
        // LOAN INFORMATION 
        let appno = (data && !_.isEmpty(data.AppNo)) ? data.AppNo : '-'
        // let product_code = (data && !_.isEmpty(data.ProductCode)) ? data.ProductCode : ''
        let program = (data && !_.isEmpty(data.Program)) ? data.Program : ''
        ...
        let guarantee = (data && !_.isEmpty(data.Guarantee)) ? data.Guarantee : ''
                        ...
                        <div key={1} className={cls['profile_div']}>
                            <div className={cls['division_container']}>
                                ...
                                <div className={`${cls['division_unset']} mt1`}>
                                    <Card className={`${cls['division_container']} ml1`}  style={{ height: '80px' }}>
                                        <div><small className="ttu">Top up (Factor)</small></div>
                            ...
                            <Card className={`${cls['division_container']}`}>
                                <div className={`${cls['division_part']} ${cls['h150']}`}>              
                                    <div className={`${cls['division_item']}`}>
                                        <div className={cls['profile_charecter']}>
                                            <div className={`${cls['division_part3']} `}>
                                                <div className="ttu b">Customer Name</div>
                                                <div>{custname}</div>
                                            </div>
                                            <div className={`${cls['division_part3']} `}>
                                                <div className="ttu b">Address</div>
                                                <div>{cust_address}</div>
                                            </div>
                                            <div className={`${cls['division_part3']}`} style={{ lineHeight: 2 }}>
                                                <div className="ttu b">Phone</div>
                                                <div>{custphone}</div>
                                            </div>
                                            <div className={`${cls['division_part3']} `}>
                                                <div className="ttu b">Business Desc</div>
                                                <div>{business}</div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div key="profile_p2" className={cls['profile_partition']}>
                            ...
                            <Card className={`${cls['profile_general']} ${cls['division_container']}`}>
                                <div className={`${cls['profile_grid']}`}>
                                    ...
                                    <div className={`${gridline_division}`}>
                                        ...
                                        <div className={`${cls['grid_item']}`}>{`${program}`}</div>{/*${product_code} - */}
                                    </div>
                                    <div className={`${gridline_division}`}>
                                    ...
                                    <div className={`${gridline_division}`}>
                                        ...
                                        <div className={`${cls['grid_item']}`}>{guarantee}</div>
                                    </div>
                                    <div className={`${gridline_division}`}>
                                    ...
                                    <div className={`${gridline_division}`}>
                                        <div className={`${cls['grid_item']} pl1 ttu b`}>Channel / Team</div>
                        ...
                        <div key={2} className={cls['profile_div']}>
                            ...
                            <div className={`${cls['timelime_container']}`}>
                                <div className={cls['timeline_title']}><small className="ttu b">Activity Timeline</small></div>
                                <Scrollbar>
...
const CRMWrapper = Form.create()(CRM)
